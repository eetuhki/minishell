...
int	invalid_chars(t_mini *mini, char input, char direction)
{
	...
	if (input == '|' && !mini->redir_open)
		mini->redir_count = 0;
	else if (mini->redir_open && ft_strchr(specials, input))
		return (FAIL);
	else if (ft_isprint(input) && mini->redir_open && input != '|' &&
		input != direction && !ft_isspace(input))
	{
		mini->redir_count = 0;
		mini->redir_open = 0;
	}
...
// validates redirection characters in more detail, allowing for << or >>, redirection not allowed at end of input 
// or followed by invalid characters, return SUCCESS or FAIL
int	validate_redir(t_mini *mini, char input, char direction)
{
	if (input == direction)
	{
		if (mini->redir_count > 0 && (input - 1) != direction)
			return (FAIL);
		mini->redir_count++;
	...
	else if (invalid_chars(mini, input, direction))
		return (FAIL);
	if (mini->redir_open && ((input + 1) == '\0'))
		return (FAIL);
	return (SUCCESS);
}

// validates redirection syntax by ignoring quotes and calling the validate_redir function.
...
int	syntax_redir(t_mini *mini, char direction)
{
	...
	while (mini->input[i])
	{
		...
		if (skip)
			i += skip;
		if (validate_redir(mini, mini->input[i], direction) == FAIL)
