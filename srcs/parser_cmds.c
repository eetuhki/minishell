...
// checks whether a token is a redir, filename/delimiter after a redir
// or if it's the first token we validate if it's a command/builtin.
// in every other case we assign token type ARG
...
static int	expand_tokens(t_mini *mini, t_cmd *cmd)
{
		...
		&& !input_is_whitespace(cmd->tokens[i].content))
	{
		if (cmd->tokens[i].type != LIMITER)
		{
			...
			if (expand_variables(mini, &cmd->tokens[i],
					&cmd->tokens[i].content) == FAIL)
				return (FAIL);
